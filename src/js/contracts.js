...
//Static binary derivative contract based on a single oracle. 
...
function pack_oracle_binary_bet_offer(C) {
    ...
    C.contract_hash = contract_hash_maker(C.oracle_start_height, C.oracle_text);
    var serialized_offer = 
        ["pair_buy_offer", C.from, C.nonce,
         C.start_limit, C.end_limit,
         C.source_id, C.source_type,
         C.contract_hash, C.new_id,
         C.amount1, C.fee1,
         C.amount2, C.fee2,
         C.subs1, C.subs2
        ];
    var signed_so = keys.sign(serialized_offer);
    return([signed_so, btoa(C.oracle_text), C.oracle_start_height]);
};
...
function contract_hash_maker(start_height, oracle_text) {
    var oracle_id = id_maker(C.oracle_start_height, 0, 0, C.oracle_text);
    var serialized_oracle_id = string_to_array(atob(oracle_id))
    var full_code = array_to_string(([2,0,0,0,32]).concat(serialized_oracle_id)).concat(static_binary_erivative);
